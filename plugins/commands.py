...
async def start(client, message):
    ...
    if len(message.command) != 2:
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        m=await message.reply_sticker("CAACAgUAAxkBAAID0Gd09g-hHQtczyVyOjkIfdCAytNyAAIJFwACiuGoVzBJhEjGlpeqHgQ") 
    ...
    if not files_:
        ...
        try:
            ...
            title = '@RKMOVIESZIP  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
