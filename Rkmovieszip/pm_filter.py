...
async def advantage_spell_chok(client, name, msg, reply_msg, vj_search):
    ...
    if vj_search == True:
        vj_search_new = False
        vj_ai_msg = await reply_msg.edit_text("<b><i>Advance Ai Of RKMOVIESZIP Try To Find Your Movie With Your Wrong Spelling.</i></b>")
        movienamelist = []
        ...
        for techvj in movienamelist:
            ...
            if mv_rqst.startswith(techvj[0]):
                await auto_filter(client, rk, msg, reply_msg, rk_search_new)
                break
        reqst_gle = mv_rqst.replace(" ", "+")
        button = [[
            InlineKeyboardButton("Gᴏᴏɢʟᴇ", url=f"https://www.google.com/search?q={reqst_gle}")
